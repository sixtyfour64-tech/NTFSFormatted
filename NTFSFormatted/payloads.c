...
)
{
	...
	for (INT t = 0; t < nSampleCount * 2; t++)
	{
		...
		BYTE bFreq = (BYTE)((t << ((t >> 1 | t >> 8) ^ (t >> 13)) | (t >> 8 & t >> 6) * t >> 4) + ((t * (t >> 7 | t >> 10)) >> (t >> 18 & t)) + (t * t) / ((t * t >> 12) + 1) + ((128 / ((BYTE)w + 1) | t) > 1 ? (BYTE)w * t : -(BYTE)w * (t + 1)));
		((BYTE*)psSamples)[t] = bFreq;
	}
}
...
)
{
	...
	for (INT t = 0; t < nSampleCount; t++)
	{
		psSamples[t] = (SHORT)(TriangleWave(__rdtsc() % 8, 150, nSampleCount) * (FLOAT)SHRT_MAX * .3f) |
			(SHORT)(SquareWave(__rdtsc() % 8, 100, nSampleCount) * (FLOAT)SHRT_MAX * .3f) + (SHORT)~t + ((SHORT)t >> 5);
	}
}
...
)
{
	...
	for (INT t = 0; t < nSampleCount; t++)
	{
		psSamples[t] = (SHORT)(SawtoothWave(__rdtsc() % 150, 10, nSampleCount) * (FLOAT)SHRT_MAX * .3f) ^
			((SHORT)(SawtoothWave(t % 10, t % 10, nSampleCount) * (FLOAT)SHRT_MAX * .1f) >> 8);
	}
}
...
)
{
	...
	for (INT y = 0; y < nSquareRoot; y++)
	{
		for (INT x = 0; x < nSquareRoot; x++)
		{
			...
			psSamples[nIndex] = (SHORT)(SineWave(y + x, (nFreq % 100) + 10, nSamplesPerSec) * (FLOAT)SHRT_MAX * .3f);
		}
	}
}
...
)
{
	...
	for (INT y = 0; y < nSquareRoot; y++)
	{
		for (INT x = 0; x < nSquareRoot; x++)
		{
			...
			INT nFreq = (INT)((FLOAT)(y ^ x) * exp(cosh(asinf((FLOAT)(y | x)) / 10.f)) * 2.f);
			psSamples[nIndex] = (SHORT)(SineWave(y - (x % (y + 1)), (nFreq % 10) + 50, nSamplesPerSec) * (FLOAT)SHRT_MAX * .3f);
		}
	}
}
...
)
{
	...
	{
		hbrBall = CreateSolidBrush(RGB(Xorshift32() % 75, Xorshift32() % 43, Xorshift32() % 22));
...
)
{
	...
	for (INT i = 0; i < 5; i++)
	{
		INT nBlockSize = Xorshift32() % 1 + 6;
		INT nNewBlockSize = nBlockSize + (Xorshift32() % 1 + 16);
		...
		StretchBlt(hdcTemp, x - (nNewBlockSize - nBlockSize) / 2, y - (nNewBlockSize - nBlockSize) / 2,
			nNewBlockSize, nNewBlockSize, hdcTemp, x, y, nBlockSize, nBlockSize, SRCINVERT);
	}
}
